...
func AddWeddingService(c echo.Context, data *dto.AddWeddingJSON) error {
	...
	go func() {
		...
		for index, file := range data.HalamanUtamaGallery {
			...
			temp_image := filepath.Join("temp_image", file.Filename)
			destinationPath := filepath.Join(path, file.Filename)
			dbpath := filepath.Join("image", file.Filename)
			...
			info, err := bucket.FPutObject(c.Request().Context(), os.Getenv("WASABI_BUCKET_NAME"), dbpath, temp_image, minio.PutObjectOptions{ContentType: "image/jpeg"})
			if err != nil {
				log.Fatalln(err)
			}

			fmt.Println("bucket info", info)
